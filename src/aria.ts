...
export function loadAndPatchAriaModule() {
  ...
  let script = readFileSync(
    path.join(__dirname, '../node_modules/aria-api/dist/aria.js'),
    { encoding: 'utf-8' }
  );
  script = script.replace(
    `var childNodes = [];`,
    `var childNodes = Array.from(node.shadowRoot?.childNodes || []).filter(n => !getOwner(n) && !isHidden(n))`
  );
...
export function loadAndPatchInjectedModule() {
  // Load aria implementation from https://github.com/xi/aria-api (MIT Licensed)
  let script = readFileSync(path.join(__dirname, '../lib/inject.js'), {
    encoding: 'utf-8',
  });
  script = script.replace(
    `var childNodes = [];`,
